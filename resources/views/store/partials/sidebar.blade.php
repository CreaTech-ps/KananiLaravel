@php $isRtl = $isRtl ?? (app()->getLocale() === 'ar'); @endphp
<aside class="w-full lg:w-64 shrink-0" id="store-sidebar" data-max-price="{{ $maxPrice ?? 1500 }}">
    <div class="sticky top-28 bg-white dark:bg-surface-darks rounded-2xl p-6 border border-slate-200 dark:border-primarys/10 shadow-sm">
        ...
        {{-- scrollbar: يمين للعربية، يسار للإنجليزية --}}
        <div class="store-sidebar-categories max-h-64 overflow-y-auto scroll-smooth" dir="{{ $isRtl ? 'ltr' : 'rtl' }}">
            <div class="space-y-2 min-w-0" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
